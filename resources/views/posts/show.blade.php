...
<div class="container mx-auto md:flex">
    <div class="md:w-1/2">
        ...
        <div>
            ...
            <p class="mt-6">{{$post->descripcion}}</p>
        </div>
        @auth
    ...
    <div class="md:w-1/2 p-5">
        <div class="shadow bg-white p-5 mb-5">
            ...
            <form action="{{route('comentarios.store',['post'=>$post,'user'=>$user])}}" method="POST">
                @csrf
                <div class="mb-5">
                    <label for="comentario" class="mb-2 block uppercase text-gray-500">
                        Añade un comentario
                    </label>
                    <textarea type="" 
                            id="comentario" 
            ...
            <div class="bg-white shadow mb-5 max-h-96 overflow-y-scroll mt-10">
                @if ($post->comentarios->count())
                    @foreach ($post->comentarios as $comentario )
                        <div class="p-5 border-gray-300 font-bold border-b">
                            <a href="{{route('post.index', $comentario->user)}}" class="font-bold">{{$comentario->user->username}}</a>
                            <p >{{$comentario->comentario}}
                                </p>
                                <p class="tetx-sm text-gray-500" >{{$comentario->created_at->diffForHumans()}}</p>
                        </div>
                    @endforeach
                @else
